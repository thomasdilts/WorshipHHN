...
class UserSearch extends User
{
    ...
    {
        return [
            [['username', 'email', 'status', 'item_name', 'display_name'], 'safe'],
        ];
    }
    ...
    {
        $query = User::find()->where(['church_id'=>Yii::$app->user->identity->church_id])->joinWith('role');

        // if user is not 'theCreator' ( You ), do not show him users with this role
        if (!Yii::$app->user->can('theCreator')) {
            $query->andFilterWhere(['!=', 'item_name', 'theCreator']);
        }
