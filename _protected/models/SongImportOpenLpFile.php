...
class ActivityExportFile extends UserActivitySearch
{
    public function exportIcs($user_id,$teamArray,$start,$end)
    {
        ...
        foreach ($activities as $activity)
        {
            ...
            $events[] = array(
                ...
                'dtstart' => Yii::$app->formatter->asDate($activity->event->start_date, "YMMddTH:mm:ss"),
                'dtend' => Yii::$app->formatter->asDate($activity->event->end_date, "YMMddTH:mm:ss"),
        ...
        file_put_contents($filePath, $ics->prepare(Church::findOne(Yii::$app->user->identity->church_id)->time_zone));

        return Yii::$app
            ->response
            ->SendFile($filePath, ActivityExportFile::GetFileName($user_id,$teamArray) . '.ics', $options = ['inline' => true, 'mimeType' => 'text/calendar']);
    }
    ...
    public function exportPdf($user_id,$teamArray,$start,$end)
    {
        ...
        return Yii::$app
            ->response
            ->SendFile($filePath, ActivityExportFile::GetFileName($user_id,$teamArray) . '.pdf', $options = ['inline' => true, 'mimeType' => 'application/pdf']);
    }
    public function exportExcel($user_id,$teamArray,$start,$end)
    {
        ...
        return Yii::$app
            ->response
            ->SendFile($filePath, ActivityExportFile::GetFileName($user_id,$teamArray) . '.xlsx', $options = ['inline' => true, 'mimeType' => 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet']);
    }
